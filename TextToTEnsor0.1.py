...
words_in_dataset = []
words_in_dataset.append(PAD)
words_in_dataset.append(GO)
words_in_dataset.append(EOS)
words_in_dataset.append(UNK)
...
def estrai_features_training(data,max_length,words_list):
    for tokenlist in data:
        ...
        rows = tokenlist.serialize().split('\n')
        #print(rows)
        if(rows[2][0] != '#'):
            j = 2
        else:
            j=3
        for i in range(j, len(rows)-2):
            features = (rows[i].split('\t')) 
            #print(features) 
            mat_token.append(features)
            sentence.append(features[1].lower())
            if (features[1].lower() not in words_list):
                words_list.append(features[1].lower())
            ...
            if (features[6] != '_'):
                position = np.int64(features[6]) - np.int64(features[0])
                if (position < 0):
...
heads_words_vocabulary = np.concatenate((vocabulary,all_relative_positions), axis = 0)
...
start = 0
train_encoder_inputs = np.empty((max_length,len(list_of_sentence)))
train_decoder_inputs = np.empty((max_length,len(list_of_sentence)))
train_target_labels = np.empty((len(list_of_sentence),max_length))
#print(head_list[0])
...
for i in range(len(list_of_sentence)):
    ...
    pad_sentence_labels = np.pad(pad_sentence_labels,(0,max_length - len(pad_sentence_labels)), 'constant', constant_values = (word_to_index.get('PAD')))
    pad_targets = np.pad(target_indixes,(0,1),'constant', constant_values = word_to_index.get('EOS'))
    pad_targets = np.pad(pad_targets,(0,max_length-len(pad_targets)), 'constant', constant_values = word_to_index.get('PAD'))
    pad_sentence = np.pad(sequence_indexes,(0,max_length - len(sequence_indexes)),'constant', constant_values = word_to_index.get('PAD'))
...
for steps in range(1,ma.ceil(len(sentence_list)/batch_size)+1): 
    ...
    try:
        feed_dict = {
        words_id: sentence_list[start:end],
        target_labels: head_list[start:end],
        pos_id: head_list[start:end],
        dec_train_imp_lengths: np.ones((batch_size), dtype=int) * decoder_hidden_dim
    }
        #slice_sentence = sentence_list[start:end]
    except:
        feed_dict = {
            words_id: sentence_list[start:],
            target_labels: head_list[start:],
            pos_id: head_list[start:],
            dec_train_imp_lengths: np.ones((batch_size), dtype=int) * decoder_hidden_dim
        }
